...
export class ProductsPage extends BasePage {
    ...
    async addProductToCartByName(productName) {
        const card = this.getProductCard(productName);
        await card.waitFor({state: 'attached'});        // Ищем ссылку "Add to cart" внутри карточки
        const addButton = card.locator('.productinfo a:has-text("Add to cart")').first();
        await addButton.waitFor({state: 'visible'});        // Прокручиваем и кликаем через evaluate для обхода оверлеев
